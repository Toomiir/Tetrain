...
class FinesseSurvivalMode:
    def __init__(self):
        ...
        self.finesse_errors = 0
        self.max_allowed_errors = 10  # Límite de errores antes de Game Over
        self.piece_start_x = self.current_piece.x
        self.horizontal_inputs_count = 0

        # Cronómetro y penalización de gravedad por errores
        self.start_ticks = pygame.time.get_ticks()
        self.elapsed_time = 0
        self.error_penalty_timer = 0
    ...
    def get_current_gravity(self):
        base_gravity = max(60, 1000 - (self.stats.level - 1) * 90)
        # Si hay errores recientes, la gravedad aumenta un 30% como penalización
        if self.error_penalty_timer > 0:
            return int(base_gravity * 0.7)
        return base_gravity
    ...
    def update(self, current_time):
        ...
        if is_grounded or self.force_lock:
            if self.lock_timer_start == 0 and not self.force_lock:
                ...
                # --- EVALUACIÓN DE FINESSE AL BLOQUEAR ---
                self.total_pieces_placed += 1
                optimal_moves = abs(self.current_piece.x - self.piece_start_x)
                if self.horizontal_inputs_count > optimal_moves:
                    self.finesse_errors += 1
                    self.error_penalty_timer = 120  # Activa penalización de gravedad por ~2 segundos
                # ------------------------------------------
                ...
                # Game Over por exceso de errores de finesse o saturación del tablero
                if not self.board.is_valid_position(self.current_piece) or self.finesse_errors >= self.max_allowed_errors:
                    print(f"¡Game Over (Finesse Survival)! Errores: {self.finesse_errors} | Score: {self.stats.score}")
                    self.board.clear()
                    self.randomizer = Randomizer()
                    self.stats = Stats() 
                    self.current_piece = self.randomizer.get_next_piece()
            ...
            self.lock_timer_start = 0
            current_gravity = self.config["soft_drop_speed"] if self.keys_held[pygame.K_DOWN] else self.get_current_gravity()
            if current_time - self.last_fall_time >= current_gravity:
                self.current_piece.move(0, 1)
                self.last_fall_time = current_time
    ...
    def draw(self, surface, font):
        ...
        # UI Izquierda (Hold, Finesse stats y Supervivencia)
        ui_left_x = 20
        ...
        draw_text(surface, f"Mode: FINESSE SURV", font, ui_left_x, stats_y - 30, (100, 200, 255))
        
        # Cálculo de porcentaje de Finesse
        finesse_pct = 100 if self.total_pieces_placed == 0 else int(((self.total_pieces_placed - self.finesse_errors) / self.total_pieces_placed) * 100)
        
        draw_text(surface, f"Finesse: {finesse_pct}%", font, ui_left_x, stats_y, (0, 255, 150))
        draw_text(surface, f"Errors: {self.finesse_errors}/{self.max_allowed_errors}", font, ui_left_x, stats_y + 30, (255, 100, 100) if self.finesse_errors > 0 else (255, 255, 255))
        draw_text(surface, f"Time: {self.elapsed_time}s", font, ui_left_x, stats_y + 60)
        draw_text(surface, f"Score: {self.stats.score}", font, ui_left_x, stats_y + 90)
        draw_text(surface, f"Lines: {self.stats.lines}", font, ui_left_x, stats_y + 120)