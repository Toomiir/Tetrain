...
class FreeplayMode:
    def __init__(self):
        ...
        # Configuración por defecto (será sobrescrita por main.py si hay configuración del usuario)
        self.config = {
            "gravity": 1000,
        ...
        self.das_active = {"left": False, "right": False}
    ...
    def handle_event(self, event, current_time):
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == kb["left"]:
                ...
                # Tecla de prueba de basura para el Freeplay
                garbage_lines = GarbageSystem.generate_garbage(1)
                self.board.add_garbage(garbage_lines)
        ...
        if is_grounded or self.force_lock:
            if self.lock_timer_start == 0 and not self.force_lock:
                ...
                if not self.board.is_valid_position(self.current_piece):
                    return True # Game Over State
        else:
            self.lock_timer_start = 0
            sd_speed = self.config.get("soft_drop_speed", 50)
            current_gravity = sd_speed if is_soft_dropping and sd_speed > 0 else self.config.get("gravity", 1000)
    ...
    def draw(self, surface, font):
        ...
        draw_text(surface, f"Mode: FREEPLAY", font, ui_left_x, stats_y - 30, (255, 100, 100))
        draw_text(surface, f"Score: {self.stats.score}", font, ui_left_x, stats_y)
        draw_text(surface, f"Lines: {self.stats.lines}", font, ui_left_x, stats_y + 30)
        draw_text(surface, f"Level: {self.stats.level}", font, ui_left_x, stats_y + 60)
        
        if self.stats.combo > 0:
            draw_text(surface, f"Combo: {self.stats.combo}", font, ui_left_x, stats_y + 100, (255, 200, 0))
        if self.stats.b2b_active:
            draw_text(surface, "B2B Active!", font, ui_left_x, stats_y + 130, (0, 255, 255))
        if self.stats.total_t_spins > 0:
            draw_text(surface, f"T-Spins: {self.stats.total_t_spins}", font, ui_left_x, stats_y + 160, (200, 100, 255))

        draw_text(surface, "Presiona [V] para basura", font, ui_left_x, stats_y + 210, (150, 150, 150))