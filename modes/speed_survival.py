...
class SpeedSurvivalMode:
    ...
    def get_current_gravity(self):
        """Gravedad ultra rápida que escala agresivamente según la cantidad de piezas colocadas"""
        return max(20, 500 - (self.pieces_placed * 15))
    ...
    def update(self, current_time):
        ...
        if is_grounded or self.force_lock:
            if self.lock_timer_start == 0 and not self.force_lock:
                ...
                self.pieces_placed += 1 # ACÁ RECUPERAMOS EL CONTADOR DE PIEZAS
            ...
            # ACÁ RECUPERAMOS LA GRAVEDAD EXTREMA
        ...
        if is_grounded or self.force_lock:
            if self.lock_timer_start == 0 and not self.force_lock:
                ...
                if not self.board.is_valid_position(self.current_piece):
                    print(f"¡Game Over (Speed Survival)! Piezas colocadas: {self.pieces_placed} | Tiempo: {self.elapsed_time}s")
                    self.board.clear()
                    self.randomizer = Randomizer()
                    self.stats = Stats() 
                    self.pieces_placed = 0
    ...
    def draw(self, surface, font):
        ...
        draw_text(surface, f"Mode: SPEED SURVIVAL", font, ui_left_x, stats_y - 30, (255, 69, 0))
        draw_text(surface, f"Time: {self.elapsed_time}s", font, ui_left_x, stats_y)
        draw_text(surface, f"Pieces: {self.pieces_placed}", font, ui_left_x, stats_y + 30, (255, 200, 0))
        draw_text(surface, f"Gravity: {self.get_current_gravity()}ms", font, ui_left_x, stats_y + 60, (255, 100, 100))
