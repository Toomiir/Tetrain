...
class TSpinSurvivalMode:
    def __init__(self):
        ...
        # Métricas específicas de T-Spins
        self.t_spin_singles = 0
        self.t_spin_doubles = 0
        self.t_spin_triples = 0
        ...
        # Generar escenario inicial propicio para T-Spins
        self.load_tspin_setup()

    def load_tspin_setup(self):
        self.board.clear()
        # Inyectamos filas con un hueco fijo para facilitar prácticas de T-Spin
        garbage = GarbageSystem.generate_garbage(4, hole_col=4)
    ...
    def update(self, current_time):
        ...
        if is_grounded or self.force_lock:
            if self.lock_timer_start == 0 and not self.force_lock:
                ...
                # Evaluación de T-Spin realizado
                if is_t_spin:
                    if lines_cleared == 1:
                        self.t_spin_singles += 1
                    elif lines_cleared == 2:
                        self.t_spin_doubles += 1
                    elif lines_cleared == 3:
                        self.t_spin_triples += 1
                ...
                if not self.board.is_valid_position(self.current_piece):
                    print(f"¡Game Over (T-Spin Survival)! Doubles: {self.t_spin_doubles} | Score: {self.stats.score}")
                    self.board.clear()
                    self.load_tspin_setup()
                    self.randomizer = Randomizer()
                    self.stats = Stats() 
            ...
            self.lock_timer_start = 0
            current_gravity = self.config["soft_drop_speed"] if self.keys_held[pygame.K_DOWN] else 1200
            if current_time - self.last_fall_time >= current_gravity:
                self.current_piece.move(0, 1)
                self.last_fall_time = current_time
    ...
    def draw(self, surface, font):
        ...
        # UI Izquierda (Hold y estadísticas de T-Spin)
        ui_left_x = 20
        ...
        draw_text(surface, f"Mode: T-SPIN SURV", font, ui_left_x, stats_y - 30, (200, 100, 255))
        draw_text(surface, f"TS Singles: {self.t_spin_singles}", font, ui_left_x, stats_y)
        draw_text(surface, f"TS Doubles: {self.t_spin_doubles}", font, ui_left_x, stats_y + 30, (255, 215, 0))
        draw_text(surface, f"TS Triples: {self.t_spin_triples}", font, ui_left_x, stats_y + 60)
        draw_text(surface, f"Time: {self.elapsed_time}s", font, ui_left_x, stats_y + 90)
        draw_text(surface, f"Score: {self.stats.score}", font, ui_left_x, stats_y + 120)