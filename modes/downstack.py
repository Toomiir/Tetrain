...
class DownstackMode:
    ...
    def update(self, current_time):
        ...
        # ACÁ RECUPERAMOS LA BASURA POR TIEMPO
        if current_time - self.garbage_incoming_timer >= self.garbage_interval:
            new_garbage = GarbageSystem.generate_garbage(1)
            self.board.add_garbage(new_garbage)
            self.garbage_lines_received += 1
