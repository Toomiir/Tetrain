...
class PCSurvivalMode:
    def __init__(self):
        ...
        # Métricas de Perfect Clear
        self.pc_attempts = 0
        self.pcs_achieved = 0
        self.pc_streak = 0
        self.pieces_used_in_current_pc = 0
    ...
    def update(self, current_time):
        ...
        if is_grounded or self.force_lock:
            if self.lock_timer_start == 0 and not self.force_lock:
                ...
                self.pieces_used_in_current_pc += 1
                ...
                if is_pc or all(cell == 0 for row in self.board.grid for cell in row):
                    self.pcs_achieved += 1
                    self.pc_streak += 1
                    self.pieces_used_in_current_pc = 0
                ...
                # Si se satura el tablero (Game Over del intento de PC)
                if not self.board.is_valid_position(self.current_piece):
                    self.pc_attempts += 1
                    print(f"¡Intento de PC Fallido! Streak perdido. Total PCs: {self.pcs_achieved}")
                    self.board.clear()
                    self.pc_streak = 0
                    self.pieces_used_in_current_pc = 0
                    self.randomizer = Randomizer()
    ...
    def draw(self, surface, font):
        ...
        # UI Izquierda (Hold y estadísticas de PC)
        ui_left_x = 20
        ...
        draw_text(surface, f"Mode: PC SURVIVAL", font, ui_left_x, stats_y - 30, (255, 215, 0))
        draw_text(surface, f"PCs Achieved: {self.pcs_achieved}", font, ui_left_x, stats_y, (0, 255, 120))
        draw_text(surface, f"Streak: {self.pc_streak}", font, ui_left_x, stats_y + 30, (255, 165, 0))
        draw_text(surface, f"Pieces Used: {self.pieces_used_in_current_pc}", font, ui_left_x, stats_y + 60)
