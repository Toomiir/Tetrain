...
class SurvivalMode:
    def __init__(self):
        ...
        # Configuraciones base de Survival
        self.config = {
            "das": DAS,
            "arr": ARR,
    ...
    def get_current_gravity(self):
        """Calcula la gravedad progresiva basada en el nivel actual (sube la velocidad cada nivel)"""
        # Nivel 1: 1000ms. Cada nivel reduce el tiempo de caída, con un límite mínimo de 60ms.
        return max(60, 1000 - (self.stats.level - 1) * 90)
    ...
    def update(self, current_time):
        ...
        if is_grounded or self.force_lock:
            ...
            # ACÁ RECUPERAMOS LA GRAVEDAD PROGRESIVA DE SURVIVAL
            current_gravity = sd_speed if is_soft_dropping and sd_speed > 0 else self.get_current_gravity()
        ...
        if is_grounded or self.force_lock:
            if self.lock_timer_start == 0 and not self.force_lock:
                ...
                # Game Over en Survival: reinicia estadísticas y cronómetro
                if not self.board.is_valid_position(self.current_piece):
                    print(f"¡Game Over! Tiempo de supervivencia: {self.elapsed_time}s | Score: {self.stats.score}")
                    self.board.clear()
                    self.randomizer = Randomizer()
                    self.stats = Stats() 
                    self.current_piece = self.randomizer.get_next_piece()
    ...
    def draw(self, surface, font):
        ...
        # UI Izquierda (Hold, Timer y Stats)
        ui_left_x = 20
        ...
        draw_text(surface, f"Mode: SURVIVAL", font, ui_left_x, stats_y - 30, (255, 165, 0))
        draw_text(surface, f"Time: {self.elapsed_time}s", font, ui_left_x, stats_y)
        draw_text(surface, f"Score: {self.stats.score}", font, ui_left_x, stats_y + 30)
        draw_text(surface, f"Lines: {self.stats.lines}", font, ui_left_x, stats_y + 60)
        draw_text(surface, f"Level: {self.stats.level}", font, ui_left_x, stats_y + 90)
        
        if self.stats.combo > 0:
            draw_text(surface, f"Combo: {self.stats.combo}", font, ui_left_x, stats_y + 130, (255, 200, 0))
        if self.stats.b2b_active:
            draw_text(surface, "B2B Active!", font, ui_left_x, stats_y + 160, (0, 255, 255))
        if self.stats.total_t_spins > 0:
            draw_text(surface, f"T-Spins: {self.stats.total_t_spins}", font, ui_left_x, stats_y + 190, (200, 100, 255))