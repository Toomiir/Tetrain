...
class ComboSurvivalMode:
    ...
    def update(self, current_time):
        ...
        if is_grounded or self.force_lock:
            if self.lock_timer_start == 0 and not self.force_lock:
                ...
                # Guardamos el estado del combo antes de actualizar
                previous_combo = self.stats.combo
                ...
                # CORRECCIÓN: El combo solo se considera roto si YA había una cadena activa (previous_combo >= 0) 
                # y esta pieza no logró limpiar líneas.
                combo_broken = (previous_combo >= 0 and lines_cleared == 0)
                ...
                if combo_broken or not self.board.is_valid_position(self.current_piece):
                    print(f"¡Game Over (Combo Survival)! Máximo combo: {max(0, previous_combo)} | Score: {self.stats.score}")
                    self.board.clear()
                    self.randomizer = Randomizer()
                    self.stats = Stats() 
                    self.current_piece = self.randomizer.get_next_piece()
            ...
            self.lock_timer_start = 0
            current_gravity = self.config["soft_drop_speed"] if self.keys_held[pygame.K_DOWN] else 1000
            if current_time - self.last_fall_time >= current_gravity:
                self.current_piece.move(0, 1)
                self.last_fall_time = current_time
    ...
    def draw(self, surface, font):
        ...
        draw_text(surface, f"Mode: COMBO SURVIVAL", font, ui_left_x, stats_y - 30, (255, 140, 0))
        
        current_display_combo = max(0, self.stats.combo)
        draw_text(surface, f"Combo: {current_display_combo}", font, ui_left_x, stats_y, (255, 220, 0))
        draw_text(surface, f"Max Combo: {self.max_combo_reached}", font, ui_left_x, stats_y + 30, (0, 255, 200))
        draw_text(surface, f"Score: {self.stats.score}", font, ui_left_x, stats_y + 60)
        draw_text(surface, f"Time: {self.elapsed_time}s", font, ui_left_x, stats_y + 90)
        
        draw_text(surface, "¡Iniciá tu combo!", font, ui_left_x, stats_y + 140, (255, 220, 80))
        draw_text(surface, "¡Luego no lo rompas!", font, ui_left_x, stats_y + 165, (255, 80, 80))